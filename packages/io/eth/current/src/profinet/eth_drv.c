//==========================================================================
//
//      src/profinet/eth_drv.c
//
//      Hardware independent networking support for ProfiNET
//
//==========================================================================
...
//#ifdef CYGSEM_IO_ETH_DRIVERS_PASS_PACKETS										// Commented
//
...
#define NUM_ETH_MSG CYGNUM_IO_ETH_DRIVERS_NUM_PKT_PN
...
/*! \fn static void eth_drv_recv(struct eth_drv_sc *sc, int total_len)
 *  \brief This function is called from a hardware driver to indicate that an input packet has arrived.
 *  \param sc pointer to a structure of type eth_drv_sc.
 *  \param total_len length of the received Ethernet frame.
 */
static void
eth_drv_recv(struct eth_drv_sc *sc, int total_len)
{
    ...
    {
//        (sc->RecData)(msg->data, (cyg_uint32) total_len);
    }
    if (msg) {
        msg->len = total_len;
        eth_drv_msg_put(&eth_msg_free, msg);
...
void eth_drv_dsr(cyg_vector_t vector,
                 ...
                 cyg_addrword_t data)
{
    ((struct eth_drv_sc *)data)->funs->deliver((struct eth_drv_sc *)data);
}
